...
RUN wget https://github.com/apache/qpid-proton/archive/0.16.0.tar.gz
RUN tar -xf 0.16.0.tar.gz
RUN tar -z -cf qpid-proton-0.16.0.tar.gz qpid-proton-0.16.0/
RUN rm -rf 0.16.0-rc1.tar.gz qpid-proton-0.16.0/
...
RUN ncftpget -u $FTP_USERNAME -p $FTP_PASSWORD -R -DD $FTP_HOSTNAME /tmp/ /web/repo/qpid-proton-stable/
RUN ncftpput -u $FTP_USERNAME -p $FTP_PASSWORD -R $FTP_HOSTNAME /web/repo/qpid-proton-stable/ /root/repo/*
